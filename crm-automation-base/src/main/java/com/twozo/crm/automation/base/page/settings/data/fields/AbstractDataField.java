...
public abstract class AbstractDataField  {
    ...
    private WebPageElement getActiveTab(final String tabName) {
        return findByXpath(String.format(CRM_LOCATOR_REGISTRY.get("crm.data.fields.active.tab"), tabName));
    }
    ...
    protected WebPageElement getActiveContactTab() {
        return getActiveTab(CRM_TEXT_REGISTRY.get("crm.contact"));
    }
    ...
    protected WebPageElement getActiveCompanyTab() {
        return getActiveTab(CRM_TEXT_REGISTRY.get("crm.company"));
    }
    ...
    protected WebPageElement getActiveDealTab() {
        return getActiveTab(CRM_TEXT_REGISTRY.get("crm.deal"));
    }
    ...
    protected WebPageElement getActiveProductTab() {
        return getActiveTab(CRM_TEXT_REGISTRY.get("crm.product"));
    }

    protected WebPageElement getAddViewCheckboxOf(final String fieldName) {
        return findByXpath(String.format(THREE_STRING_FORMAT, OPEN_PARENTHESIS, getFieldBlock(fieldName), FieldAttribute.ADD_VIEW_CHECKBOX));
    }

    protected WebPageElement getRequiredCheckboxOf(final String fieldName) {
        return findByXpath(String.format(THREE_STRING_FORMAT, OPEN_PARENTHESIS, getFieldBlock(fieldName), FieldAttribute.REQUIRED_CHECKBOX));
    }
    ...
    public WebPageElement getLimitExceededForCustomField() {
        return findByText("Maximum limit of 10 custom fields per type has been reached");
    }
    ...
    protected WebPageElement getAddSystemFieldsButton() {
        return findByText(DataFieldsButton.SYSTEM_FIELDS);
    }
     ...
     */
    protected WebPageElement getCheckboxOf(final String fieldName) {
        waitTillVisible(getMenuBlock(fieldName));

        return findByXpath(formatTwoStrings(getMenuBlock(fieldName), CRM_LOCATOR_REGISTRY.get("checkbox")));
    }
    ...
    protected WebPageElement getAddSelectedFieldsButton() {
        return findByText(DataFieldsButton.ADD_SELECTED_FIELDS);
    }
    ...
    protected WebPageElement getCustomFieldName() {
        return findByXpath(FieldAttribute.CUSTOM_FIELD_NAME);
    }
    ...
    protected WebPageElement getSelectCustomFieldType() {
        return findByXpath(FieldAttribute.CUSTOM_FIELDS_FIELD_TYPE);
    }
    ...
    protected WebPageElement getCustomFieldAddButton() {
        return findByXpath(FieldAttribute.ADD_BUTTON);
    }
    ...
    protected WebPageElement getChoice() {
        return findByText(FieldAttribute.CHOICE);
    }
    ...
    protected WebPageElement getAddChoice() {
        return findByText(FieldAttribute.ADD_CHOICE);
    }
    ...
    protected WebPageElement getNonDraggableElement(final Field field) {
        return findByXpath(formatTwoStrings(getFieldBlock(field.getName()), FieldAttribute.NON_DRAGGABLE));
    }

    protected WebPageElement getDraggableElement(final Field field) {
        return findByXpath(formatTwoStrings(getFieldBlock(field.getName()), FieldAttribute.DRAGGABLE));
    }

    protected WebPageElement getFieldName(final Field field) {
        final String name = field.getName();

        return findByXpath(formatTwoStrings(getFieldBlock(name), XPathBuilder.getXPathByText(name)));
    }

    protected WebPageElement getFieldType(final Field field) {
        return findByXpath(formatTwoStrings(getFieldBlock(field.getName()), XPathBuilder.getXPathByText(field.getFieldType())));
    }
    ...
    protected WebPageElement getAddSystemFieldSearchBar() {
        return findByXpath(CRM_LOCATOR_REGISTRY.get("crm.data.fields.search"));
    }
     ...
     */
    protected Collection<WebPageElement> getSystemFieldSearchResults() {
        return findElementsByXpath(FieldAttribute.SYSTEM_FIELD_SEARCH_RESULT);
    }
     ...
     */
    protected boolean isFieldElementDisplayed(final String fieldName, final String elementName) {
        final String xpath = formatTwoStrings(getFieldBlock(fieldName), elementName);
    ...
    protected boolean isDependableFieldElementDisplayed(final String fieldName, final String elementName) {
        final String xpath = formatTwoStrings(getDependableBlock(fieldName), elementName);
    ...
    protected boolean areAllChoicesPresent(final String[] options) {
     ...
     * false otherwise.
     */
    protected boolean checkFieldAttributes(final Record field) {
    ...
    public boolean addCustomField(final FieldStatus fieldStatus) {
        ...
        return isDisplayed(findByXpath(fieldBlock)) && isDisplayed(findByXpath(formatTwoStrings(fieldBlock, XPathBuilder.getXPathByText(fieldStatus.getFieldType()))));
    }
    ...
    public void checkMaximumLimit(final FieldStatus fieldStatus) {
        ...
        waitTillClickable(FieldAttribute.CUSTOM_FIELD_NAME);
        send(getCustomFieldName(), customFieldName);
        waitTillClickable(FieldAttribute.CUSTOM_FIELDS_FIELD_TYPE);
        ...
        waitTillClickable(FieldAttribute.ADD_BUTTON);
    ...
    public boolean verifySearchResult(final String fieldName) {
        ...
        for (final WebPageElement webPageElement : getSystemFieldSearchResults()) {

            if (!getText(webPageElement).equalsIgnoreCase(contains(fieldName))) {
                return false;
            } else {
                return !getText(findByXpath(FieldAttribute.SYSTEM_FIELD_SEARCH_RESULT)).equals("No Results");
            }
        }
    ...
    protected void verifySystemFieldNotEditable(final Field field) {
        ...
        click(findByXpath(FieldAttribute.UPDATE_BUTTON));
    }
    ...
    public boolean checkIfDisplayed(final FieldStatus fieldStatus) {
        ...
        isDisplayed(findByXpath(formatTwoStrings(fieldBlock, XPathBuilder.getXPathByText(fieldType))));

        final boolean isChecked = isSelected(addViewCheckbox);
        final boolean isRequired = isSelected(requiredCheckbox);

        if (draggable) {
            isDisplayed(findByXpath(formatTwoStrings(fieldBlock, FieldAttribute.DRAGGABLE)));
        } else {
            isDisplayed(findByXpath(formatTwoStrings(fieldBlock, FieldAttribute.NON_DRAGGABLE)));
        }

        if ((addView && !isChecked) || (!addView && isChecked)) {
            click(addViewCheckbox);
            click(findByXpath(formatTwoStrings(fieldBlock, FieldAttribute.UPDATE_BUTTON)));
        }
        ...
        if (editable) {
            isDisplayed(findByXpath(formatTwoStrings(fieldBlock, FieldAttribute.DELETE_ICON)));
        } else {
            try {
                if (isDisplayed(findByXpath(formatTwoStrings(fieldBlock, FieldAttribute.DELETE_ICON)))) {
        ...
        if (hideable) {
            isDisplayed(findByXpath(formatTwoStrings(fieldBlock, FieldAttribute.EYE_ICON)));
        } else {
            try {
                if (isDisplayed(findByXpath(formatTwoStrings(fieldBlock, FieldAttribute.EYE_ICON)))) {
...
//    }
    ...
    public boolean editCustomField(final FieldStatus fieldStatus) {
        ...
        if (isFieldPresent(actualName)) {
            ...
            click(findByXpath(formatTwoStrings(fieldBlockXpath, FieldAttribute.EDIT_ICON)));
    ...
    public boolean deleteField(final String fieldName) {
        ...
        if (isFieldPresent(fieldName)) {
            ...
            hoverByXpath(fieldBlock);
            click(findByXpath(formatTwoStrings(fieldBlock, FieldAttribute.DELETE_ICON)));
    ...
    public boolean verifyEyeIcon(final Field field) {
        ...
        hoverByXpath(fieldBlock);
        click(findByXpath(formatTwoStrings(fieldBlock, FieldAttribute.EYE_ICON)));
        click(getAddSystemFieldsButton());

        return !isSelected(getCheckboxOf(fieldName));
    }
    ...
    public boolean hideAutoGeneratingSystemField(final Field hideableSystemField) {
        final String fieldName = hideableSystemField.getName();
        final String fieldBlock = getFieldBlock(fieldName);
    ...
    public boolean verifyAutoGeneratingSystemFieldNotEditable(final Field field) {
        ...
        click(getAddViewCheckboxOf(fieldName));
        click(findByXpath(FieldAttribute.UPDATE_BUTTON));
        click(findByText(FieldAttribute.CANCEL_BUTTON));
    ...
    public boolean removeSystemField(final String fieldName) {
        ...
        final WebPageElement checkbox = getCheckboxOf(fieldName);
    ...
    public boolean addAllSystemFields() {
        ...
        for (final Field specificField : getAllFields()) {
            ...
            try {
                checkbox = getCheckboxOf(fieldName);
    ...
    protected void uncheck(final String[] mandatoryFields) {
        ...
        for (final String specificDiv : mandatoryFields) {
            ...
            final WebPageElement path = getCheckboxOf(specificDiv);

            if (isSelected(path)) {
                click(path);

                if (isDisplayed(findByXpath(formatTwoStrings(getFieldBlock(specificDiv), FieldAttribute.UPDATE_BUTTON)))) {
                    click(findByXpath(formatTwoStrings(getFieldBlock(specificDiv), FieldAttribute.UPDATE_BUTTON)));
                }
            }
        }
    }
    ...
    public boolean addAndRemoveSystemField(final Field fieldToBeAdded,
        ...
        final WebPageElement systemFieldToBeAdded = getCheckboxOf(fieldToBeAddedToList);
        final WebPageElement systemFieldToBeRemoved = getCheckboxOf(fieldToBeRemovedFromList);

        if (!isSelected(systemFieldToBeAdded)) {
            click(getCheckboxOf(fieldToBeAddedToList));
        }
        if (isSelected(systemFieldToBeRemoved)) {
            click(getCheckboxOf(fieldToBeRemovedFromList));
        }
        return true;
    }
...
//        }
    ...
    public void addParticularFieldToList(final String fieldName) {
        click(findByXpath(String.format(FieldAttribute.SPECIFIC_FIELD_COLUMN_SETTINGS_CHECKBOX, fieldName)));
    }
    ...
    protected boolean isNonDraggableIconDisplayed(String divBlockName) {
        return isDisplayed(findByXpath(formatTwoStrings(divBlockName, FieldAttribute.NON_DRAGGABLE)));
    }
    ...
    public boolean verifyEyeIconIsNotVisibleForDefaultFields() {
        for (Field value : getDefaultFields()) {
            ...
            hoverByXpath(buildXpathByText(name));

            try {
                WebPageElement element = findRightElement(List.of(
                        new Element(LocatorType.XPATH, formatTwoStrings(getFieldBlock(name), buildXpathByText("Required")), true),
...
//        waitTillClickable("//*[@class='MuiStack-root twozo-css-prefix-1gqdsmd']");
    ...
    public boolean isAddViewCheckBoxEditableForMandatoryField() {
        for (final String fieldDiv : getMandatoryFields()) {
            click(findByXpath(formatTwoStrings(fieldDiv, FieldAttribute.ADD_VIEW_CHECKBOX)));

            try {
                if (isDisplayed(findByXpath(formatTwoStrings(fieldDiv, FieldAttribute.UPDATE_BUTTON)))) {
    ...
    public boolean enableAddViewForAllSystemFields() {
        ...
        for (final Field field : getAllFields()) {
            ...
            try {
                addViewCheckbox = getAddViewCheckboxOf(fieldName);
    ...
    public void disableRequiredForNonMandatoryFields() {
        ...
        for (final String field : getFields()) {
            ...
            if (!mandatoryFields.contains(field)) {
                ...
                if (isSelected(getRequiredCheckboxOf(field))) {
                    click(getRequiredCheckboxOf(field));

                    try {
                        click(findByXpath(formatTwoStrings(getFieldBlock(field), FieldAttribute.UPDATE_BUTTON)));
                    } catch (Exception exception) {
                        LOGGER.info(field);
                    }
                }
     ...
     * @param fieldName The name of the field to be added.
     */
    protected void addSystemField(final String fieldName) {
        waitTillVisible(XPathBuilder.getXPathByText(DataFieldsButton.SYSTEM_FIELDS));
        click(getAddSystemFieldsButton());
        waitTillVisible(getMenuBlock(fieldName));
        try {
            Thread.sleep(1000);
    ...
    protected void addSystemFields(final List<String> fields) {
        waitTillVisible(XPathBuilder.getXPathByText(DataFieldsButton.SYSTEM_FIELDS));
        ...
        if (!fieldsToBeAdded.isEmpty()) {
            ...
            for (final String field : fieldsToBeAdded) {
                ...
                path = formatTwoStrings(getMenuBlock(field), "//*[@type='checkbox']");
    ...
    public boolean addSystemField(final FieldStatus fieldStatus) {
        ...
        final String fieldTypeXPath = formatTwoStrings(getFieldBlock(fieldName), XPathBuilder.getXPathByText(fieldType));
    ...
    public boolean enableAddViewForAutoGeneratingField(final FieldStatus fieldStatus) {
        ...
        if (!isSelected(getAddViewCheckboxOf(fieldName))) {
            ...
            try {
                shortWaitTillVisible(formatTwoStrings(fieldBlock, FieldAttribute.UPDATE_BUTTON));
            } catch (Exception exception) {
            }
        }
    ...
    private Collection<String> getFieldsEnabledAs(final String addViewOrRequired) {
        ...
        for (int i = 1; i <= elementsByXpath.size(); i++) {
            ...
            if (isSelected(fieldElement)) {
                final Collection<WebPageElement> matchingElements = findElementsByXpath(formatTwoStrings(fieldBlock, "//*[@aria-label]"));
    ...
    protected boolean setDefaultChoice(final boolean isDependableField, final String fieldName, final String choice) {
        ...
        if (!isDependableField) {
            click(findByXpath(formatTwoStrings(getFieldBlock(fieldName), "//div[3]//p")));
        } else {
            click(findByXpath(formatTwoStrings(getDependableBlock(fieldName), "//div[3]//p")));
        }
        ...
        if (Objects.equals("Mark as Default Choice", currentLabel)) {
            ...
            click(findByXpath(CRM_LOCATOR_REGISTRY.get("body")));
            click(findByXpath(FieldAttribute.UPDATE_BUTTON));
            click(findByXpath(formatTwoStrings(getFieldBlock(fieldName), XPathBuilder.getXPathByText("14"))));
