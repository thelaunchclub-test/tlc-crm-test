package com.twozo.page.list.view;
...
public class ListView extends BasePage {
    ...
    public boolean isListEmpty() {
        waitTillVisible(LOCATORS.get("tbody"));

        try {
            return isDisplayed(findByXpath(LOCATORS.get("crm.list.view.empty")));
    ...
    public int getCount() {
        ...
        if (isListEmpty()) {
            ...
            return findElementsByXpath(LOCATORS.get("crm.list.view.count")).size();
        }
    }

    public int getFieldPositionInColumnSettings(final String fieldName) {
        final List<WebPageElement> elements = new ArrayList<>(
                findElementsByXpath(LOCATORS.get("crm.column.settings.field.names"))
        );
    ...
    public String getFieldData(final int listNumber, final int fieldPosition) {
        ...
        return getText(findByXpath(format(String.format(LOCATORS.get("crm.list.view.specific.row"), listNumber),
                String.format(LOCATORS.get("crm.list.view.specific.field.data"), fieldPosition + 1))));
    }
}
