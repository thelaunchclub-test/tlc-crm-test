package com.twozo.crm.automation.page.settings.data.fields;

import com.twozo.crm.automation.page.settings.Settings;
import com.twozo.crm.automation.page.settings.data.fields.contact.field.ContactField;
import com.twozo.crm.automation.page.settings.data.fields.field.DependableField;
...
public abstract class AbstractDataField extends Settings {
    ...
    protected WebPageElement getAddViewCheckboxOf(final String fieldName) {
        return findByXpath(String.format(THREE_STRING_FORMAT, OPEN_PARENTHESIS, getFieldBlock(fieldName), FieldElement.ADD_VIEW_CHECKBOX));
    }

    protected WebPageElement getRequiredCheckboxOf(final String fieldName) {
        return findByXpath(String.format(THREE_STRING_FORMAT, OPEN_PARENTHESIS, getFieldBlock(fieldName), FieldElement.REQUIRED_CHECKBOX));
    }
    ...
    public boolean isLimitExceededNotificationDisplayed() {
        final String notification = XPathBuilder.getXpathByContains(CRM_TEXT_REGISTRY.get("crm.notification.maximum"));
    ...
    protected WebPageElement getCheckboxOf(final String fieldName) {
        ...
        return findByXpath(format(getMenuBlock(fieldName), CRM_LOCATOR_REGISTRY.get("checkbox")));
    }
     ...
     */
    protected String getFieldBlock(final String fieldName) {
        return String.format(FieldElement.FIELD_BLOCK, fieldName);
    }
    ...
    protected WebPageElement getNonDraggableElement(final String fieldName) {
        return findByXpath(format(getFieldBlock(fieldName), FieldElement.NON_DRAGGABLE));
    }

    protected WebPageElement getDraggableElement(final String fieldName) {
        return findByXpath(format(getFieldBlock(fieldName), FieldElement.DRAGGABLE));
    }

    protected WebPageElement getFieldName(final String fieldName) {
        return findByXpath(format(getFieldBlock(fieldName), XPathBuilder.getXPathByText(fieldName)));
    }

    protected WebPageElement getFieldType(final String fieldName, final String fieldType) {
        return findByXpath(format(getFieldBlock(fieldName), XPathBuilder.getXPathByText(fieldType)));
    }
    ...
    protected boolean areChoicesPresent(final List<String> options) {
     ...
     * @param fieldName The name of the field.
     */
    protected void verifySystemFieldNotEditable(final String fieldName) {
    ...
    public boolean editCustomField(final FieldStatus fieldStatus) {
        ...
        String fieldBlockXpath;

        if (isFieldPresent(fieldName)) {
            fieldBlockXpath = getFieldBlock(fieldName);
     ...
     * </p>
     *
     * @param fieldName The {@link Field}.
     * @return true if the 'eye' icon is displayed, false otherwise.
     */
    public boolean verifyEyeIcon(final String fieldName) {
        final String fieldBlock = getFieldBlock(fieldName);
     ...
     * @return true if the field is hidden successfully, false otherwise.
     */
    public boolean hideAutoGeneratingSystemField(final String hideableSystemField) {
     ...
     */
    public boolean verifyAutoGeneratingSystemFieldNotEditable(final String fieldName) {
        if (!isFieldPresent(fieldName)) {
            addField(fieldName);
        }
    ...
    public boolean addAllSystemFields() {
        ...
        for (final String specificField : getAllFields()) {
        ...
        try {
            for (final String specificField : getAllFields()) {
                isDisplayed(findByXpath(specificField));
            }
        } catch (Exception exception) {
            return false;
        }
     ...
     */
    protected void uncheck(final List<String> mandatoryFields) {
     ...
     */
    public boolean addAndRemoveSystemField(final String fieldToBeAdded, final String fieldToBeRemoved) {
        click(getAddSystemFieldsButton());
...
//        }
    ...
    public boolean verifyEyeIconIsNotVisibleForDefaultFields() {
        for (String value : getDefaultFields()) {

            hover(new Element(LocatorType.XPATH, buildXpathByText(value), true));

            try {
                WebPageElement element = findRightElement(List.of(
                        new Element(LocatorType.XPATH, format(getFieldBlock(value), buildXpathByText("Required")), true),
...
//        waitTillClickable("//*[@class='MuiStack-root twozo-css-prefix-1gqdsmd']");
    ...
    public boolean enableAddViewForAllSystemFields() {
        ...
        for (final String field : getAllFields()) {
            LOGGER.info(field);
     ...
     */
    private List<String> getFieldsEnabledAs(final String addViewOrRequired) {
    ...
    public List<String> getFieldsEnabledAsAddView() {
     ...
     */
    protected abstract Collection<String> getDefaultFields();
     ...
     */
    protected abstract List<String> getAllFields();
